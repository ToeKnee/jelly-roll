...
class ContactInfoForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if shop.in_country_only and areas and areas.count() > 0:
            ...
            billing_state = (
                contact and getattr(contact.billing_address, "state", None)
            ) or selection
            shipping_state = (
                contact and getattr(contact.shipping_address, "state", None)
            ) or selection
        ...
        billing_country = (
            contact and getattr(contact.billing_address, "country", None)
        ) or self._default_country
        shipping_country = (
            contact and getattr(contact.shipping_address, "country", None)
        ) or self._default_country
    ...
    def _check_state(self, data, country):
        if (
            country
            and config_value("SHOP", "ENFORCE_STATE")
            and country.adminarea_set.filter(active=True).count() > 0
        ):
            if not data or data == selection:
                raise forms.ValidationError(
                    self._local_only
                    and _("This field is required.")
                    or _("State is required for your country.")
                )
            if (
                country.adminarea_set.filter(active=True)
                .filter(
                    Q(name=data)
                    | Q(abbrev=data)
    ...
    def clean_ship_addressee(self):
        if not self.cleaned_data.get("ship_addressee") and not self.cleaned_data.get(
            "copy_address"
        ):
            first_and_last = " ".join(
                (
    ...
    def ship_charfield_clean(self, field_name):
        ...
        return self.fields["ship_" + field_name].clean(
            self.data.get("ship_" + field_name)
        )
