...
class Shipper(BaseShipper):
    ...
    def cost(self):
        ...
        return self.carrier.price(weight)
...
class Carrier(models.Model):
    ...
    def price(self, wgt):
        ...
        prices = self.tiers.filter(expires__isnull=False, min_weight__lte=wgt).exclude(expires__lt=datetime.date.today())
        if not prices.count() > 0:
            prices = self.tiers.filter(expires__isnull=True, min_weight__lte=wgt)
...
class WeightTier(models.Model):
    ...
    def __unicode__(self):
        return u"WeightTier: %s @ %s" % (self.price, self.weight)
