...
from satchmo.currency.api.serializers import (
    CurrencySerializer,
    CurrencySessionSerializer,
)
...
class CurrencySerializerTest(TestCase):
    ...
    def test_get_latest_exchange_rate(self):
        ...
        self.assertEqual(
            serializer.get_latest_exchange_rate(currency), exchange_rate.rate
        )
...
class CurrencySessionSerializerTest(TestCase):
    ...
    def test_get__currency_doesnt_exist(self):
        ...
        self.assertEqual(
            serializer.errors, {"iso_4217_code": ["BTC is not an accepted currency"]}
        )
