...
def send_order_update(order_status):
    ...
    text_templates = [
        "email/order/status/{slug}.txt".format(slug=email_slug),
        "email/order/status/generic.txt",
    ]
    text_template = loader.select_template(text_templates)
    html_templates = [
        "email/order/status/{slug}.html".format(slug=email_slug),
    ...
    subject = _(
        "Your {shop_name} order #{order_id} has been updated - {status}"
    ).format(
    ...
    message = EmailMultiAlternatives(subject, body, shop_email, [customer_email])
