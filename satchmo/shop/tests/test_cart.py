...
class CartTest(TestCase):
    ...
    def test_cart_adding_errors(self):
        ...
        py_shirt.save()
        response = self.client.post(
            prefix + "/cart/add/", {"productname": "PY-Rocks", "quantity": "1"}
        )
        self.assertContains(
            response,
        ...
        self.assertContains(
            response, "Please enter a whole number.", count=1, status_code=200
        )
        ...
        self.assertContains(
            response, "Please enter a positive number.", count=1, status_code=200
        )
