            ...
            if opt == "qty":
                if value != prod.items_in_stock:
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        "Updated %s stock to %s" % (key, value),
                    )
                ...
                if value != full_price:
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        "Updated %s unit price to %s" % (key, value),
                    )
...
class VariationManagerForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.product:
            ...
            for grp in configurableproduct.option_group.all():
                optchoices = [
                    ("%i_%i" % (opt.option_group.id, opt.id), opt.name)
            ...
            for opts in configurableproduct.get_all_options():
                ...
                kw = {"initial": None, "label": " ".join(optnames), "required": False}
    ...
    def _create_variation(self, opts, key, data, request):
        ...
        v = self.product.configurableproduct.create_variation(
            opts, name=nameval, sku=skuval, slug=slugval
        )
