...
def quick_product(context, product, show_wishlist=True):
    ...
    context.update({"product": product, "show_wishlist": show_wishlist})
    return context
...
def full_product(context, product):
    ...
    context["other_prices"] = [
        money_format(
            convert_to_currency(product.unit_price, currency.iso_4217_code),
            currency.iso_4217_code,
        )
        for currency in Currency.objects.filter(accepted=True)
        .exclude(iso_4217_code=current_currency)
        .order_by("iso_4217_code")
    ]
    return context
