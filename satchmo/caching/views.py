...
class CacheDeleteForm(forms.Form):
    ...
    children = forms.ChoiceField(
        label=_('Include Children?'), choices=YN, initial="Y")
    kill_all = forms.ChoiceField(
        label=_('Delete all keys?'), choices=YN, initial="Y")
...
stats_page = user_passes_test(lambda u: u.is_authenticated(
) and u.is_staff, login_url='/accounts/login/')(stats_page)
...
view_page = user_passes_test(lambda u: u.is_authenticated(
) and u.is_staff, login_url='/accounts/login/')(view_page)
...
delete_page = user_passes_test(lambda u: u.is_authenticated(
) and u.is_staff, login_url='/accounts/login/')(delete_page)
