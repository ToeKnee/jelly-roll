...
from satchmo.shop.models import OrderItem, OrderPayment
        ...
        if item.product.is_subscription:  # TODO - need to add support for products with trial but non-recurring
            ...
            if item.expire_date == datetime.date(datetime.now()) and item.completed:
                if item.id == OrderItem.objects.filter(product=item.product, order=item.order).order_by('-id')[0].id:
                    ...
                    new_order_item = OrderItem(order=item.order, product=item.product, quantity=item.quantity,
                                               unit_price=item.unit_price, line_item_price=item.line_item_price)
                    ...
                    if payments.payment not in ipn_based and item.order.balance > 0:
                        ...
                        if results:
                            ...
                            item.order.add_status(
                                status='Pending', notes="Subscription Renewal Order successfully submitted")
                            new_order_item.completed = True
                            new_order_item.save()
                            orderpayment = OrderPayment(
                                order=item.order, amount=item.order.balance, payment=str(payment_module.KEY.value))
                            orderpayment.save()
    return HttpResponse()
